      ...
      <div class="WrapComponent DataWrap">
        <v-card class="CardComponent DataCard" elevation="2.5">
          <div id="DataInput">
            ...
            ></v-file-input>
            <!-- ------윤식추가------ -->
            <div><b>Data Shape</b><br>Rows :{{RawData_row}} <br>Columns :{{RawData_col}} </div>
            <!-- ------윤식추가------ -->
          </div>
        </v-card>
        <div class="StepName">
      ...
      <div class="WrapComponent TransformWrap">
        <v-card class="CardComponent TransformCard" elevation="1">
          <div class="TransformRadio">
            ...
            <label for="radio4"> K-mean Clustering</label><br>
            ...
            <input type="radio" @change="setTransformMode" name="transform" id="radio6" value="6" v-model="picked">
            <label for="radio6"> PCA</label><br>
            <input type="radio" @change="setTransformMode" name="transform" id="radio7" value="7" v-model="picked">
            <label for="radio7"> Normalization</label><br>
            <input type="radio" @change="setTransformMode" name="transform" id="radio8" value="8" v-model="picked">
            <label for="radio8"> Min-max Scaling</label><br>
          </div>
...
//const FEATURE_GRAPH_WIDTH='200px';
  export default {
    ...
    data: () => ({
      isComponent : true,
      country: null,
      countries: ['LSTM', 'ANN', 'DNN', 'DCNN', 'S-ARIMA', 'SVM'],
      RawData_row:0,
      RawData_col:0,
      dlResult: '',
      rawData:undefined,
        ...
        reader.onload = async (e) => {
          ...
          .then((jsonObj)=>{
            ...
            axios.post('http://localhost:5000/calPC',{datas:jsonObj}).then((r)=>{
              //***------------윤식추가----------***
              let LineChartJsonData = JSON.parse(r.data.LineChartJson);
              this.RawData_row = r.data.row;
              this.RawData_col = r.data.col;
              //***------------윤식추가----------***
              let corr = JSON.parse(r.data.result)[0]
              this.featureDataLoaded = true;
              // console.log(this.LineChartLoaded);
              ...
              //***------------윤식추가----------***
              EventBus.$emit('message',LineChartJsonData);
              //***------------윤식추가----------***
